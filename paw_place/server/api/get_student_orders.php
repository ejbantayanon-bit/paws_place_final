...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    try {
        // If student is GUEST, only show orders from current session
        if ($student_id === 'GUEST') {
            ...
            $stmt = $conn->prepare("SELECT order_id, pre_order_code, final_code, customer_name, order_source, total_amount, status, time_placed, time_paid FROM orders WHERE order_id IN ($ids_placeholder) ORDER BY time_placed DESC");
            $stmt->bind_param($types, ...$guest_ids);
        } else {
            // Normal student/employee lookup
            $stmt = $conn->prepare("SELECT order_id, pre_order_code, final_code, customer_name, order_source, total_amount, status, time_placed, time_paid FROM orders WHERE student_id = ? ORDER BY time_placed DESC LIMIT 50");
        ...
        while ($order = $ordersRes->fetch_assoc()) {
            ...
            $itemStmt = $conn->prepare("
                SELECT oi.order_item_id, oi.menu_item_id, mi.name, oi.quantity, oi.price_at_sale, oi.modifiers
                FROM order_items oi
